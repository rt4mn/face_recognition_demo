...
def get_name_to_photo_mapping(filepath, Global):
    '''
    Read the file named in filepath as a yaml file.
    This returns a dict keyed by name, with the image path name as value.
    Walk all entries to populate the global list.
    '''
    known_face_encodings = []
    known_face_names = []
    with open(filepath) as input:
        datamap = yaml.safe_load(input)
    for name, imagefile in datamap.items():
        image = face_recognition.load_image_file(imagefile)
        face_encoding = face_recognition.face_encodings(image)[0]
        known_face_encodings.append(face_encoding)
        known_face_names.append(name)
        print(f'DEBUG: face {name} loaded')
...
if __name__ == '__main__':
    ...
    get_name_to_photo_mapping('photomap.yaml', Global)
